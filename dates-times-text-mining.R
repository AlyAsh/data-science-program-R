...
# unnest_tokens() extracts individual words and other meaningful chunks of text.
...
# Use extract on column "source" to get the devices used after the pattern "Twitter for "
trump_tweets %>%
    extract(source, "source", "Twitter for (.*)") %>%
    count(source)

# We are interested in what happened during the campaign dates 
# between the day Trump announced his campaign and election day.
campaign_tweets <- trump_tweets %>%
      extract(source, "source", "Twitter for (.*)") %>%
      filter(source %in% c("Android", "iPhone") & # source is either Android or iPhone
              created_at >= ymd("2015-06-17") & # campaign start date
...
# based on the extract hour (EST) it was tweeted.
